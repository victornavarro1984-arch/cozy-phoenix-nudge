      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground">
            Manage projects, move tasks through completion pipelines, and check off milestones.
          </p>
        </div>
              ...
              <div className="space-y-3 flex-1 overflow-y-auto max-h-[calc(100vh-280px)] pr-1">
                {colTasks.map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className="bg-card text-card-foreground border border-border/70 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-indigo-500/40 transition-all space-y-3 group"
                    >
                          ...
                          <div className="space-y-1 pt-1">
                            {task.subtasks.map((st) => (
                              <div
                                ...
                                className="flex items-center gap-2 text-xs font-medium cursor-pointer hover:text-indigo-600<dyad-write path="src/components/kanban/KanbanBoard.tsx" description="Kanban board with integrated edit task modal and filter workflows">
"use client";
...
export const KanbanBoard = () => {
  ...
  const [isTaskModalOpen, setIsTaskModalOpen] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const [selectedPriority, setSelectedPriority] = useState<string>('all');<dyad-write path="src/components/kanban/KanbanBoard.tsx" description="Kanban board with integrated edit task modal and filter workflows">
"use client";
