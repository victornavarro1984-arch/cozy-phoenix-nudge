...
export const OverviewTab = () => {
  ...
  const handleSaveReflection = (e: React.FormEvent) => {
    ...
    toast.success('Reflection logged! Growth mindset point added ✨');
    setReflectionInput('');
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Daily Habit Quick Check & Reflection */}
        <div className="space-y-6">
          ...
          <Card className="rounded-2xl border border-border/60 bg-gradient-to-br from-indigo-500/5 via-background to-cyan-500/5">
            <CardContent className="p-4">
              <form onSubmit={handleSaveReflection} className="space-y-2">
                <div className="flex items-center gap-2 text-xs font-bold text-indigo-600 dark:text-indigo-400">
                  <Brain className="w-4 h-4" />
                  <span>Daily Mindset Reflection</span>
                </div>
                <p className="text-xs text-muted-foreground">
                  "What is your single highest-leverage focus for today?"
                </p>
                <div className="flex items-center gap-2 pt-1">
                  <Input
                    placeholder="e.g. Ship the API authentication ADR..."
                    value={reflectionInput}
                    onChange={(e) => setReflectionInput(e.target.value)}
                    className="rounded-xl text-xs bg-background"
                  />
                  <Button type="submit" size="sm" className="bg-indigo-600 text-white rounded-xl text-xs shrink-0">
